...
public class ManagerImple extends AbstractManage {
	...
	private int initPhenixThreadLimit() {
		//从数据库查询
		DictionaryInfo dictionaryInfo=SpringUtil.getBean(DictionaryService.class).getDictionaryByKey(RecommendConstant.RECOMMEDN_THREAD_LIMIT);
		int threadLimit=RecommendConstant.RECOMMEDN_THREAD_DEFAULT_LIMIT;
		if(null!=dictionaryInfo && null!=dictionaryInfo.getValue()){
			threadLimit=Integer.valueOf(dictionaryInfo.getValue());
		}
		return threadLimit;
	}

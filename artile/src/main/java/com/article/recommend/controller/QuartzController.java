...
public class QuartzController {
    ...
    public String updateQuartz(QuartzInfo quartzInfo,RedirectAttributes redirectAttributes){
    ...
    public String resume( QuartzInfo quartzInfo){
        quartzInfo=quartzService.getQuartzInfoById(quartzInfo.getId());
        if(quartzInfo.getState().equals(RecommendConstant.SYSTEM_DATE_INVALID)) {//失效
            return JSON.toJSONString("请先恢复数据为有效状态！");
        }
        try {

...
public final class RecommendFactory {
   ...
   /* public static ClusterSimilarity clusterSimilarity(SIMILARITY type, UserSimilarity us) throws TasteException {
    ...
    public static Recommender itemRecommender(DataModel dataModel,final ItemSimilarity is, boolean pref) throws TasteException {
		if (pref) {
...
*/
  ...
  /*  public static RecommenderBuilder treeClusterRecommender(final ClusterSimilarity cs, final int n) throws TasteException {
     ...
     * @throws TasteException
     */
    public static double evaluate(EVALUATOR type, RecommenderBuilder rb, DataModelBuilder mb, DataModel dm, double trainPt,Double countPre) throws TasteException {
        //1.0 全部数据  trainPt（训练数据比例，1-trainPt=测试数据比例）
        Double score= buildEvaluator(type).evaluate(rb, mb, dm, trainPt, countPre);
     ...
     * statsEvaluator
     */
    public static IRStatistics statsEvaluator(RecommenderBuilder rb, DataModelBuilder mb, DataModel m, int topn,Double countPre) throws TasteException {
        RecommenderIRStatsEvaluator evaluator = new GenericRecommenderIRStatsEvaluator();
        IRStatistics stats = evaluator.evaluate(rb, mb, m, null, topn, GenericRecommenderIRStatsEvaluator.CHOOSE_THRESHOLD, countPre);
